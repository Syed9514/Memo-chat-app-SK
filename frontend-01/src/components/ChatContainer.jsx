          ...
          return (
            <div key={message._id}>
              ...
              <div className={`flex w-full mb-1 ${isMyMessage ? "justify-end" : "justify-start"}`}>
                <div className={`flex gap-3 items-end ${isMyMessage ? "flex-row-reverse" : "flex-row"}`}>
                  ...
                  {/* 2. CONTENT COLUMN */}
                  <div className={`flex flex-col ${isMyMessage ? "items-end" : "items-start"} overflow-hidden`}>
                    
                    {/* Message Bubble */}
                    <div 
                      className={`
                        relative px-4 py-2.5 shadow-sm rounded-2xl flex flex-col
                        /* --- FIX: Explicit Width Constraints --- */
                        w-fit
                        max-w-[200px]
                        sm:max-w-xs
                        md:max-w-md
                    ...
                    >
                      ...
                      {message.image && (
                        <img
                          src={message.image}
                          alt="Attachment"
                          className="max-w-[150px] rounded-lg mb-2 border border-black/5"
                        />
                      )}
                      
                      {/* Text & Time Wrapper */}
                      <div className="flex flex-wrap items-end gap-2 align-bottom">
                        {message.text && (
                          <p className="text-sm sm:text-[15px] leading-relaxed whitespace-pre-wrap break-words min-w-0">
                            {message.text}
                          </p>
                        )}
